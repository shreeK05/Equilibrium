...
import { TaskInput, ConstraintInput, FixedCommitment, ScheduleBlock } from '../src/scheduler/types';
import { calculatePriority } from '../src/scheduler/priority';
...
describe('Equilibrium Extended Core Scheduler Test Suite', () => {
  ...
  describe('1. SLEEP SHIELD & 13. HARD-CONSTRAINT WALL', () => {
    it('Never intersects normal or overnight sleep', () => {
      ...
      const customConstraints: ConstraintInput = {
        ...constraints, sleepStart: '21:00', sleepEnd: '06:00'
      };
      const t = createTask('t1', 180, customEnd);
      const res = runReschedulerPipeline([t], customConstraints, [], [], horizonStart, customEnd, localNow);
    ...
    it('Task starts exactly at sleep end', () => {
      ...
      const res = runReschedulerPipeline([t], constraints, [], [], horizonStart, horizonEnd, localNow);
  ...
  describe('2. FIXED COMMITMENTS', () => {
    it('Handles adjacent and overlapping fixed commitments', () => {
      ...
      const res = runReschedulerPipeline([t], constraints, fixed, [], horizonStart, horizonEnd, now);
  ...
  describe('3. DEADLINES', () => {
    it('Never schedules a task after its deadline', () => {
      ...
      const res = runReschedulerPipeline([t], constraints, [], [], horizonStart, horizonEnd, now);
  ...
  describe('4. CAPACITY', () => {
    it('Zero available capacity', () => {
      const fixed: FixedCommitment[] = [{ id: 'f1', start: now, end: horizonEnd }];
  ...
  describe('5. KNAPSACK', () => {
    it('Optimizes value correctly', () => {
      ...
      const res = runReschedulerPipeline([t1, t2, t3], constraints, [], [], horizonStart, d, now);
    ...
    it('Tie-breaking fallback to ID', () => {
      ...
      const res = runReschedulerPipeline([t1, t2], constraints, [], [], horizonStart, d, now);
  ...
  describe('7. PARTIAL SCHEDULING & 9. INFEASIBLE SCHEDULE', () => {
    it('Partially schedules and explicitly reports deferred workload', () => {
      ...
      const res = runReschedulerPipeline([t], constraints, [], [], horizonStart, d, now);
      expect(res.blocks.length).toBe(1);
  ...
  describe('8. RESCHEDULING', () => {
    it('Preserves completed/locked blocks', () => {
      ...
      const res = runReschedulerPipeline([t], constraints, [], [lockedBlock], horizonStart, horizonEnd, newNow);
  ...
  describe('10. DETERMINISM', () => {
    it('Produces identical results 100 times', () => {
      ...
      const firstRun = runReschedulerPipeline([t1, t2], constraints, [], [], horizonStart, horizonEnd, now);
      const firstStr = JSON.stringify(firstRun.blocks);
      
      for(let i=0; i<100; i++) {
        const run = runReschedulerPipeline([t1, t2], constraints, [], [], horizonStart, horizonEnd, now);
  ...
  describe('14. VALIDATOR AS FINAL SAFETY NET', () => {
    it('Catches corrupted schedules', () => {
      ...
      // Negative duration
      const badBlock1: ScheduleBlock = { taskId: 't1', type: 'TASK', start: now, end: now, durationMinutes: -30, isLocked: false };
      expect(validateSchedule([badBlock1], constraints, [], [t])).toBe(false);

      // Sleep overlap
      const badBlock2: ScheduleBlock = { taskId: 't1', type: 'TASK', start: new Date('2026-10-14T23:30:00.000Z'), end: new Date('2026-10-15T00:30:00.000Z'), durationMinutes: 60, isLocked: false };
      expect(validateSchedule([badBlock2], constraints, [], [t])).toBe(false);

      // Exceeds remaining
      const badBlock3: ScheduleBlock = { taskId: 't1', type: 'TASK', start: now, end: new Date('2026-10-14T10:00:00.000Z'), durationMinutes: 120, isLocked: false };
      expect(validateSchedule([badBlock3], constraints, [], [t])).toBe(false);

      // Overlaps fixed
      const fixed: FixedCommitment[] = [{ id: 'f1', start: now, end: new Date('2026-10-14T09:00:00.000Z') }];
      const badBlock4: ScheduleBlock = { taskId: 't1', type: 'TASK', start: new Date('2026-10-14T08:30:00.000Z'), end: new Date('2026-10-14T09:30:00.000Z'), durationMinutes: 60, isLocked: false };
      expect(validateSchedule([badBlock4], constraints, fixed, [t])).toBe(false);

      // Deadline violation
      const badBlock5: ScheduleBlock = { taskId: 't1', type: 'TASK', start: new Date('2026-10-22T08:30:00.000Z'), end: new Date('2026-10-22T09:30:00.000Z'), durationMinutes: 60, isLocked: false };
      expect(validateSchedule([badBlock5], constraints, [], [t])).toBe(false);
    });
  });
  ...
  describe('11. PROPERTY / RANDOMIZED TESTING', () => {
    it('Generates valid schedules for 50 randomized inputs', () => {
      ...
      for(let iter = 0; iter < 50; iter++) {
        ...
        const res = runReschedulerPipeline(tasks, constraints, [], [], horizonStart, horizonEnd, now);
        
        // Assert invariants via validator
        const valid = validateSchedule(res.blocks, constraints, [], tasks);
