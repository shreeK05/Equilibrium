...
describe('Equilibrium Core Scheduler', () => {
  ...
  it('A. Sleep Shield Invariant - Never overlaps', () => {
    ...
    const result = runReschedulerPipeline(tasks, constraints, [], [], horizonStart, horizonEnd, now);
    ...
    for (const b of result.blocks) {
      if (b.type === 'TASK') {
        const overlaps = intervalsIntersect(b.start, b.end, sleepStart, sleepEnd);
  ...
  it('B. Fixed Commitments Protection', () => {
    ...
    for (const b of result.blocks) {
      if (b.type === 'TASK') {
        const overlaps = intervalsIntersect(b.start, b.end, fixed[0].start, fixed[0].end);
        expect(overlaps).toBe(false);
      }
    }
  });
  ...
  it('C. Capacity and E. Knapsack Optimal Selection (0/1 Determinism)', () => {
    ...
    const result = runReschedulerPipeline(tasks, constraints, [], [], horizonStart, customEnd, now);
  ...
  it('D. Priority Engine components breakdown', () => {
    ...
    const p = calculatePriority(task, now); // 1 hour to deadline
  ...
  it('Phase 2 Acceptance Test - E2E Disruption', () => {
    ...
    const result1 = runReschedulerPipeline(tasks, constraints, fixed, [], horizonStart, new Date('2026-10-19T00:00:00.000Z'), now);
    ...
    const tasksDisrupted: TaskInput[] = [
    ...
    const result2 = runReschedulerPipeline(tasksDisrupted, constraints, fixed, lockedBlocks, horizonStart, new Date('2026-10-19T00:00:00.000Z'), newNow);
